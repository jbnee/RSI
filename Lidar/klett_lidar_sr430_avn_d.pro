  ...
  For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
    ...
    fn=Data_path+yr+'\'+month+'\'+ dnm+'.'+ni+'D'
     ...
     bk[jr]=mean(datab[bnum-200:bnum-1])
      Pr2D[jr,*]=(datab(0:bnum-1)-bk[jr])*ht^2
       FOR Im=0,bnum-1 do begin

        if (Pr2D[Jr,im] LE 0) then Pr2D[jr,im]=min(bk[jr])
      endfor
...
FOR im=0 , nx-AVN,AVN do begin
  ...
  For iy=0,999 do begin
    AVSig[Jm,iy]=MEAN(Pr2D[im:im+avn-1,iy])
...
stop
    ...
    ; title='X= log(Pr2D))',xtitle='X',ytitle='km'
...
; evaluation of the second bottom term below
  ...
  ENDFOR   ;nj
    ...
    stop ;secion IIB
     ...
     outfile1=out_path+fnm+'D_BETA.bmp'
       write_bmp,outfile1,tvrd(/true)
    OTx=out_path+fnm+'betaD.txt'
    openw,2,OTx
    printf,2,beta_a
    close,2
      ...
      ; AOD1[RS2]=0
       ...
       outfile1=out_path+fnm+'AOD_D.bmp'
       write_bmp,outfile1,tvrd(/true)
