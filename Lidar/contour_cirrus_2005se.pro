...
;read,h1,h2, prompt='height range as,10,30:  '
 ...
 FOR n=ni,nf do begin
  ...
  cmin=500+min(pr2m[nf-ni-5,0:1000]);10 is arbitrary set the 5th file
  cmax=1*max(pr2m[nf-ni-5,0:1000]);100000;60000
  nlevs_max=40
...
i0=1
 plot_position1 = [0.1,0.20,0.90,0.45]; plot_position=[0.1,0.15,0.95,0.45]
 plot_position2 = [0.1,0.65,0.90,0.90]; plot_position2=[0.1,0.6,0.95,0.95]
 BAR_POSITION1=[0.96,0.2,0.98,0.45]
 BAR_POSITION2=[0.96,0.65,0.98,0.90]


;plot,pr2m,y,background=-2,color=2

 contour,pr2m,x,y,xtitle='Parallel time',ytitle='km',xrange=[1,nx],yrange=[h1,h2], LEVELS = CLEVS, /FILL,$
...
; stop
 ...
 FOR nd=ni+1,nf do begin
  ...
  ENDFOR
  col = 240 ; don't change it
  cmind=cmin/4 ;min(pr2d[nf-ni-5,0:1000]);5 is arb set
  cmaxd=cmax/4 ;max(pr2d[nf-ni-5,0:1000])/4;100000;60000
  nlevs_max=40
  nlevs_max = 20 ; choose what you think is right
 ...
 NLEVS = N_ELEMENTS(CLEVS)+1 ;actual number of countour levels
    ...
    contour,pr2d,x,y,xtitle='Perpendicular time',ytitle='km',title=yr+dnm,xrange=[1,nx], yrange=[h1,h2],LEVELS = CLEVS, /FILL,$
    C_COLORS = C_INDEX, color=2,/FOLLOW,position=plot_position2,xticks=1
   ;xyouts,500,5,'fn',color=1,charsize=2
 ;stop
 ; plot a color bar, use the same clevs as in the contour
    ;nlevs=40
  ...
  cntrname =bpath+'cirrus plots\'+yr+dnm+'.png'
...
stop
pname =bpath+'cirrus outs\'+yr+dnm+'_plt.png'
