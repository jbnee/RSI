  ...
  For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
   ...
   ; out_file=data_path+'\output\'+'_prf.'+'bmp'
    ...
    ; cnt_sig=cnt_sig(0:bnum-1)
     ...
     FOR Id=0,hi_bin do begin

        if (X0[jr,Id] eq 0 AND Id GT 10)  then X0[jr,Id]=X0[jr,Id-1]
     endfor



     if (Jr eq 0) then begin
        plot,X0[jr,*],ht,color=2,background=-2,yrange=[h1,h2],title='SR430_PR2_d',xtitle= 'pr2_d',ytitle='km';file 1',color=2
     endif else begin
...
b2=hi_bin-1
  ...
  For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
   ...
   beta_a=1.e6*beta_a  ; convert to Mega_meter
    ...
    xtitle='beta_a D ',  title='D back scattering coeff02- JA18210915-40 AV3'
   ...
   ; wait,1
    stop
     BETAM=beta_a[*,0:hi_bin] ;
      out_filx=OUT_path+dnm+'beta_D.txt'
      openw,1,out_filx
      printf,1,betaM

      out_file1=OUT_path+dnm+'D_beta.bmp'
...
stop
    bratio[0,0:hi_bin]=1+smooth(beta_a[0,0:hi_bin]/1000000./beta_r[0:hi_bin],10)
     ...
     stop    ;Section III
      out_file2=out_path+dnm+'BK_D_ratio.'+'bmp'
