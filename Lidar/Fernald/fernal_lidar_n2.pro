...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 ; n5=nx/5  ;average 5 profiles
  ...
  beta_a=dblarr(nx,bnum)  ;backscattering coefficient of aerosol
  bratio=dblarr(nx,bnum)
  ...
  For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
   ...
   ; out_file=data_path+'\output\'+'_prf.'+'bmp'
    ...
    ; readf,1,cnt_sig  ;for LICEL
     ...
     plot,X[jr,*],ht,color=2,background=-2,xrange=[0,1e10],yrange=[0,h2],title='PR2_m',xtitle= 'pr2_m',ytitle='km';file 1',color=2
     endif else begin
...
;5 files are averaged
 ...
 BTM=dblarr(nx,bnum)  ; 分母  denumerator of Fernald eq. (5) and (6)
 BTM1=dblarr(nx)  ; first term in BTM
 BTM2=dblarr(nx,bnum)  ;2nd term in BTM
...
; evaluation of the second bottom term below
   b2=hi_bin
  For nj=0,nx-1  do begin  ;from starting file n1 to last file n2
   ;sumBTM2[nj]=0
   ...
   Endfor  ;k
    ...
    avm_beta_a=total(beta_a,1)/nx ;average beta_a for m channel
