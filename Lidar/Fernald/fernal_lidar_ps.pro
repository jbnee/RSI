...
;Part 2 Read data
 ...
 ; month=strmid(dnm,0,2)
  ...
  ;define arrays
  DATAB=FLTARR(2,bnum)
  PS0=fltarr(NF,bnum) ;original PS  Analog signal
  PS=fltarr(NF,bnum); treated PS signal
  PS1=fltarr(NF/2,bnum) ;PS1=fltarr(m/2,Nbin) ; parallel
  PS2=fltarr(NF/2,bnum) ; perpendicular
  ...
  ;read,h1,h2,prompt='height region PS 1, 6 km  '
  ; input data
...
; average process
   ...
   FOR i=0 , NF/2-1, m0 do begin; average 5 profiles
     ...
     For jy=0,bnum-1 do begin ;sum 5
      ...
      ; PS3[J2,jy]=MEAN(PS3[i:i+m0-1,jy])
      PS3[j2,jy]=(PS12[i,jy]+PS12[i+1,jy]+PS12[i+2,jy]+PS12[i+3,jy]+PS12[i+4,jy])/5
...
stop;check PS3;
      ...
      ;bk=min(PS3[Jr,*]);  treat background
       bk(J1)=min(PS3[J1,bnum-200:bnum-1]);
      ;endfor; J1

      ;FOR J2=0,nf/(2*M0)-1 DO BEGIN  ;fit bk
      ; Y=poly_fit(n,bk,2);
      ; B=poly(n,Y);
       ;PS3[J2,*]=(PS3[j2,*]-B[j2]); *(z^2)
       pr2M[J1,*]=(PS3[j1,*]-bk[j1])*(z^2)
 ...
 ;
;Part II Backscattering coefficient calculation  bPSed on Fernald 1984
...
; evaluation of the second bottom term below
  For nj=0,m5-1  do begin  ;from starting file n1 to lPSt file n2
   ;
   ...
   ENDFOR   ;nj
   stop
   plot,smooth(beta_a[1,*],20),ht[b1:b2],color=2,background=-2,title=' find max ht for beta '
    ...
    zb=fltarr(2,nlevs);
     ...
     CONTOUR, zb, xb, yb,position=BAR_0, xrange=[0,1],/xstyle,xtickv=findgen(2),$
      ...
      xticks=1,xtickname=xname,yrange =[cmin,cmax],/YSTYLE, $
       ycharsize=0.7, charsize=1,/noerPSe,title='cc'
