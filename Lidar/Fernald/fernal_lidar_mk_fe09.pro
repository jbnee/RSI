  ...
  For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
   ...
   Endfor  ;k
    ...
    xyouts,0.00002,4,'1st and average backscattering coefficient',color=2
     ...
     m_beta_a=beta_a[*,lo_bin:hi_bin-1]; redefine
     sz=size(m_beta_a)
      For IS1=0,sz[1]-1 do begin  ;remove negative values
        For IS2=1,sz[2]-1 do begin
        if (m_beta_a[IS1,IS2] LE 0) then begin
        m_beta_a[IS1,IS2]=m_beta_a[IS1,IS2-1]  ;beta_r[IS2]
      ...
      plot,bratio[0,0:hi_bin],ht,color=1,xrange=[0,20],yrange=[0,h2],background=-2,title=dnm,xtitle='bacattering ratio',ytitle='km'
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ...
   ; out_file=data_path+'\output\'+'_prf.'+'bmp'
    ...
    ; cnt_sig=cnt_sig(0:bnum-1)
     bk2=min(cnt_sig[bnum-200:bnum-1]);  treat background
...
; evaluation of the second bottom term below
  For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
   ...
   Endfor  ;k
    ...
    d_beta_a=beta_a[*,lo_bin:hi_bin-1] ;redefine for d channel

     For IS1=0,sz[1]-1 do begin  ;remove negative values
        For IS2=1,sz[2]-1 do begin
         if (d_beta_a[IS1,IS2] LE 0) then begin
         d_beta_a[IS1,IS2]=d_beta_a[IS1,IS2-1];beta_r[IS2]
       ...
       bratio[0,0:hi_bin-1]=1+smooth(d_beta_a[0,0:hi_bin-1]/beta_r[0:hi_bin-1],10)
       plot,bratio[0,0:hi_bin-1],ht,color=1,background=-2,xrange=[0,20],yrange=[0,h2],title=dnm,xtitle='Perpendicular bacattering ratio 0',ytitle='km'

       for Ld=1,n5-1 do begin
       bratio[Ld,0:hi_bin-1]=1+smooth(d_beta_a[Ld,0:hi_bin-1]/beta_r[0:hi_bin-1],10)
       endfor ;Ld
    ...
    ;
     ...
     ; av_m=total(m_beta_a,1)/n5  ; ave m_beta_a
      dep_ratio=d_beta_a[*,100:hi_bin-1]/(d_beta_a[*,100:hi_bin-1]+m_beta_a[*,100:hi_bin-1]); [0:hi_bin]/(av_d[0:hi_bin]+av_m[0:hi_bin])
      bkdp=mean(dep_ratio);[hi_bin-100:hi_bin])
      dep_ratio=dep_ratio-bkdp;
      Av_dep_ratio=smooth(total(dep_ratio,1),50)
      plot,av_dep_ratio,ht,color=3,thick=2,background=-2;xrange=[0,1],yrange=[0,h2],xtitle='ave-depol,ratio',ytitle='km'
