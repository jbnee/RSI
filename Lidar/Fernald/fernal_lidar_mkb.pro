...
;5 files are averaged
 ...
 BTM=dblarr(n5,b2)  ; 分母  denumerator of Fernald eq. (5) and (6)
 ...
 ;sumbtm2=fltarr(n5)
    ...
    ;BTM1[*]=0         ; X[*,1250]/((beta_r[1250]+beta_a[*,1250]/1.E-6))
     ...
     beta_a(*,hi_bin)=0;  beta_r(hi_bin-1)  ;initial condition at top  =beta_a
...
; evaluation of the second bottom term below
 ...
 For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
   ...
   BTM1[nj]=X[nj,hi_bin]/(beta_r[b2]+beta_a[nj,hi_bin])
   ...
   For k =hi_bin-1 ,1,-1 do begin   ;calculation start from hi_bin
     ...
     ;BTM2[nj,k]=Sa*(X[nj,k]+X[nj,k -1]*exp((A[k]+A[k-1])/2)*dz1000
      BTM2[nj,k]=BTM2[nj,k-1]+Sa*(V[nj,k]+(V[nj,k]+V[nj,k+1])/2)*dz1000
    ...
    ;ENDFOR ;k
     ...
     sz=size(m_beta_a)
      For IS1=1,sz[1]-1 do begin
       ...
       plot,avm_beta_a[n5/2,*]/maxa,ht,xrange=[0,40],yrange=[h1,h2], color=2, background=-2 ,xtitle='beta_a',ytitle='km'
...
;b2=hi_bin
  For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
   ...
   For k =hi_bin-1 ,1,-1 do begin   ;calculation start from hi_bin
     A[k]=-2*(Sa-Sr)*INTB[k]  ;(beta_r[k]+beta_r[k-1])*dz1000
     V[nj,k]=X[nj,k]*exp(A[k])
     BTM2[nj,k]=BTM2[nj,k-1]+Sa*(X[nj,k]+X[nj,k -1]*exp(A[k]+A[k-1]))*dz1000
     BTM[nj,k]=BTM1[nj]+BTM2[nj,k]
     beta_a[nj,k]=(V[nj,k])/(BTM[nj,k])-(beta_r[k]);
   Endfor  ;k
    ...
    maxb=max(beta_a[0,*])
    plot,beta_a[0,0:hi_bin],ht,color=1,yrange=[0,h2],xrange=[0,maxb],background=-2,xtitle='d_beta_a'
    ...
    stop
     ...
     maxb=max(d_beta_a[30,hi_bin-lo_bin])/5
