...
; Part I Rayleigh calcution
 ; read,bnum, prompt='Enter total number bins (eg.4096)  '
  ...
  tau=fltarr(bnum); Rayleigh optical thickness
   TM=fltarr(bnum)  ;  Rayleigh transmission
      ;density=fltarr(bnum+1);air density
      ;density= 1.E25*(2.4498-0.22114*ht+0.00701*ht^2-7.75225E-5*ht^3)  ;ht in km; in molec/m3
     N_air=density(ht) ;
     beta_r=5.45E-32*(550./532.)^4*N_air ;density(ht)  ;Rayleigh backscattering coefficient
...
;TM[0]=TM[1]
TM[bnum-1]=TM[bnum-2]
plot,TM,ht,background=-2,color=35,xrange=[0,1],xtitle='tau & TM',ytitle='km';charsize=2.0,title='Optical thickness'
xyouts,0.02,15,'tau optical thickness'
stop

oplot,tau,ht,color=145;xrange=[0.75,1],ytitle='km',xtitle='transmission';
xyouts,0.35,15,'tau:left; Transmission:right',color=2,charsize=1.5
stop
INTB=fltarr(bnum+1); integral of beta_r from top down
h0=bnum*dz1000
INTB[bnum]=0;beta_r[bnum]
;SINTB=fltarr(h0); integral of beta_r from top down
;SINTB[bnum]=0;beta_r[bnum]
FOR I1=bnum-1,0,-1 do begin
 hx=I1*dz
 ;INTB[I1]=INTB[I1+1]+(beta_r[hx]+beta_r[hx+dz1000])*(dz1000/2)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ;Data_path="D:\lidar_files\Depolar\";\Lidar_systems\LidarPro\DePolar\";  2003\jn022058\jn022058.4M""   ; AUR or SPR
   ...
   out_path='F:\lidar_data\temp_3\'; lidarPro\output\yrmn"
  yr=''
  ...
  For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
   ...
   ; out_file=data_path+'\output\'+'_prf.'+'bmp'
    ...
    ; plot,cnt_sig
     ...
     bk1=min(smooth(sg1[jr,bnum-200:bnum-1],20));
...
;5 files are averaged
 ...
 ;sumbtm2=fltarr(n5)
       ;Initial condition
    ;BTM1[*,b2]=0
            ; X[*,1250]/((beta_r[1250]+beta_a[*,1250]/1.E-6))
...
; evaluation of the second bottom term below
   ;;;;; INTB[I1]=INTB[I1+1]+(beta_r[I1]+beta_r[I1-1])*(dz1000/2)
   A[hi_bin-1]=2*(Sa-Sr)*INTB[hi_bin-1]
   V[*,hi_bin-1]=X[*,hi_bin-1]*exp(A[hi_bin-1]);
    beta_a[*,hi_bin]=beta_r[hi_bin]
 For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
     BTM1[nj]=X[nj,hi_bin]/(beta_r[hi_bin]+beta_a[nj,hi_bin])
     ;BTM2[nj,hi_bin+1]=0
     BTM2[nj,hi_bin-1]=0
   For k =b2-2 ,1,-1 do begin   ;calculation start from hi_bin
     A[k]=2*(Sa-Sr)*INTB[k];   *take positive values; (beta_r[k]+beta_r[k-1])*dz1000/2
     V[nj,k]=X[nj,k]*exp(A[k])  ; numerator term
     BTM2[nj,k]=BTM2[nj,k+1]+(V[nj,k]+V[nj,k+1])*dz1000/2
   ...
   b3=600
   m_beta_a=fltarr(n5,b3+1)
  ;V=V[*,1:b3] ; throw away the edge terms
 ; BTM=BTM[*,1:b3]
 ; h0=50 ; ignore 50 bins below
  Vv=V[*,1:600]; h0:hi_bin];
  BBTM=BTM[*,1:600];  h0:hi_bin]
  m_beta_a=Vv/BBTM; ;-beta_r[lo_bin:hi_bin]
   ...
   ;  beta_a[rmv,*]=0
     ...
     ; m_beta_a=beta_a[*,0:b3]; redefine
      ...
      ; plot,m_beta_a[n5/2,*]/maxa,ht,xrange=[0,6],yrange=[h1,h2], color=2, background=-2 ,xtitle='beta_a',ytitle='km'
        ...
        stop
    out_file1=out_path+fnm+'mk3_AOD.'+'bmp'
       write_bmp,out_file1,tvrd()
       OT1=out_path+fnm+'m_beta_mk3.txt'
       openw,2,OT1
       printf,2,m_beta_a
       close,2
