Pro Fernal_cirrus2; In the first part, read data and plot;
...
;Part II Backscattering coefficient calculation  based on Fernald 1984
 ...
 density= 1.E25*(2.4498-0.22114*km+0.00701*km^2-7.75225E-5*km^3)  ;ht in km; density in molec/m3
...
;plot,beta_r,ht, background=-2,color=2
 Izt= 1250             ;top bin at 30 km which is 1250
 A[0]=0
 For na=1,Izt do begin
  A[na]=A[na-1]+(Sa-Sr)*(beta_r[na]+beta_r[na+1])*dz   ;Integral formula
 endfor  ;na
  ;A[Izt]=A[Izt-1]   ;set A[1250]=A[1249] ;counting starting from 0 to 1249

 plot,A[0:1249],km, background=-2,color=1,xtitle='A(z)'

 stop
 bnum=Izt
 beta_a(*,Izt)=5*beta_r(Izt)  ;initial condition at 30 km for beta_a
;read,IZt,prompt='top height to treat in bin number: 1250(30 km),833(20km0  :'
  ...
  For nj=0,n2-n1  do begin  ;from starting file n1 to last file n2
    ...
    X[nj,0:Izt-1]= sg[nj,0:Izt-1]*ht^2/TO3   ;PR^2
   ...
   ;plot,beta_a[0,*],ht,background=-2,color=1
    ...
    plot,beta_a[0,100:izt-100],ht[100:1150]/1000.,background=-2,color=1,xtitle='beta_a'

      stop
      For k=0,n2-n1 do begin
         oplot,beta_a[k,100:izt],ht[100:1150]/1000.,color=2

       endfor ;k
       stop
    bratio=1+beta_a[1,100:Izt]/beta_r[100:izt]

     plot,bratio,ht[100:1150]/1000,background=-2,color=1,xrange=[0,200]
      bbratio=bratio
      stop
      sumratio=0
       For L=1,n2-n1 do begin
       bratio=1+beta_a[L,100:Izt]/beta_r
       oplot,bratio+ max(bbratio),ht[100:1150]/1000.,color=1
       ;stop
       sumratio=sumratio+bratio
       endfor
     stop
     avratio=sumratio/(L+1)
     oplot,avratio,ht[100:1150]/1000,color=1, psym=7
     stop
