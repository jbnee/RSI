...
; Part I Rayleigh calcution
 ; read,bnum, prompt='Enter total number bins (eg.4096)  '
   ...
   device, decomposed=0
   Loadct,5
   bT=160  ;50      ;160 ns for SR430 bin width
...
;;;;;input data
  For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
   ...
   ; out_file=data_path+'\output\'+'_prf.'+'bmp'
     ...
     ; readf,1,cnt_sig   ;LICEL
     cnt_sig=read_binary(fn, DATA_TYPE=2)  ; SR430 ;read binary file
    ; plot,cnt_sig
     close,1
     cnt_sig=cnt_sig[0:1999]
      ; treat background
   ...
   ;
     ...
     mxa=max(X0)
     plot,X0[0,*],ht,color=2,background=-2,xrange=[0,mxa*5],yrange=[0,h2],title='PR2_m',xtitle= 'X0:pr2_m',ytitle='km';file 1',color=2
...
For iy=0,999 do begin                  ; height or range counter
X1[J2,iy]=MEAN(X0[i:i+4,iy])           ;mean of 5 files
;x2[J2,IY]=(X0[i,iy]+x0[i+1,iy]+X0[i+2,iy]+X0[i+3,iy]+X0[i+4,iy])/5
endfor;  iy
;X1[14,*]=(X1[12,*]+X1[12,*]+X1[13,*])/3.0  this is for Fe14 2009
;X1[15,*]=(X1[16,*]+X1[17,*])/2.
X1[J2,*]=smooth(X1[J2,*],20);/10
oplot,X1[J2,*]*(1+J2*0.5),ht,color=iy*30
;print,j2
wait,1
j2=j2+1
endfor ;i files all
  ...
  For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
   ...
   beta_a=V/BTM
     ...
     ;READ,rmv,prompt='remove files'
       ...
       out_path='F:\lidar_data\output2018\';Fernald\'
      ...
      ; maxa=max(m_beta_a[n5/2,0:b2-b1-1])/5  ; 30 is arbitraryavm_beta_a=total(beta_a,1)/n5 ;average beta_a for m channel
       ...
       ;xAOD=total(m_beta_a,2)*SA*dz1000 ; this is the AOD of each set of profile
       ;AOD=total(xAOD,1)/n5
       AOD1=fltarr(n5)
       AOD1[0]=0
       ...
       AOD1[ia]=total(m_beta_a[ia,bc1:bc2],2)*dz1000*SA
       ENDFOR;ia
    ...
    h=ht[h0:b2-1]
     ...
     ;;;;;;;;;;;;;;;;;;;;renormalize beta_b to remote negative values
      ...
      FOR Ib=0,n5-1 do begin
       ...
       TAOD[Ib]=total(Tbeta,2)*dz1000*SA
