...
;Part 2 Read data
  close,/all
  fnm=''
  Read, fnm, PROMPT='Enter filename as 0414 AM ASC;'
  ;da='0806 PM '
  da=strmid(fnm,2,3);  '0805 '
  month=strmid(fnm,0,2)
 ; da='0415ASC'
...
;n2=750
 ...
 ;;;;;;;;;;;;;;;;;;;; reading data below
  ...
  FOR I=0,n2-n1-1 DO BEGIN ; open file to read
      ...
      AS[nc,*]=DATAB[a_p,*];  Take analog or photon data
...
; evaluation of the second bottom term below
  For nj=0,m5-1  do begin  ;from starting file n1 to last file n2
   ...
   Endfor  ;k
    ...
    plot,beta_a[2,0:b2],ht,color=2,background=-2,yrange=[h1,h2],xrange=[0,maxa*10],$
    ...
    ; out_file=DATA_path+fnm+'_beta.png'

    ; write_png,out_file,tvrd()
  stop

  EXT=beta_a(*,0:1999)*Sa;  Extinction coefficient

;;; calculate AOD ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

  AOD=total(Ext,2)*dz1000

  plot,AOD,color=2,background=-2,title='AOD'
    stop
      xnm=strmid(fnm,1,6)
      ;out_path='f:\rsi\lidar\Fernald\test\'; lidarPro\output\yrmn"
      out_file1=data_path+xnm+'AOD.png'
      write_png,out_file1,tvrd(/true)

      out2=data_path+xnm+'AOD.txt'
      openw,2,out2
      printf,2,AOD

      out3=data_path+xnm+'1_beta.txt'
      hd=''
      hd1=size(Ext)
      ...
      format1='(E16.4)'
     ;
      printf,2,EXT,format=format1
      close,2
      stop
      xx=indgen(hd1[1])
      yy=indgen(hd1[2])*dz
      contour_make,ext,xx,yy
