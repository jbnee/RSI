PRO axes, XVALS=xvals, XLABELS=xlabels, Rot=rot,YVALS=yvals, YLABELS=ylabels, $
...
IF (map_type EQ 8) THEN BEGIN
 ...
 FOR i=0, npts-1 DO BEGIN
  ...
  IF ((mydeg EQ 180) OR (mydeg EQ -180)) THEN xlabels(i)='180'
...
IF ((map_type EQ 1) OR (map_type EQ 2) AND (sector NE 1)) THEN BEGIN
 latmin=!map.ll_box(0)
 lonmin=(!map.ll_box(1)+360.0+rot) MOD 360.0 ; Convert longitudes to 0 to 360.
 latmax=!map.ll_box(2)
 lonmax=(!map.ll_box(3)+360.0+rot) MOD 360.0 ; Convert longitudes to 0 to 360.
 ...
 FOR i=0, npts-1 DO BEGIN
  ...
  IF (mydeg EQ 0) THEN label='0'
  IF ((MYDEG EQ -180) OR (MYDEG EQ 180)) THEN label='180'
