PRO cont, FIELD=field, X=xpts, Y=ypts, AUTOLEVS=autolevs, NOCOLBAR=nocolbar, NOFILL=nofill,$
...
IF (N_ELEMENTS(CB_WIDTH) EQ 0) THEN cb_width=100
IF (N_ELEMENTS(CB_HEIGHT) EQ 0) THEN cb_height=100
irreg=0
...
IF KEYWORD_SET(HASH) THEN BEGIN
 ...
 IF (!guide.levels_npts NE 2) THEN BEGIN
  ...
  STOP
 ENDIF
 IF (N_ELEMENTS(SPACING) EQ 0) THEN spacing=0.8
