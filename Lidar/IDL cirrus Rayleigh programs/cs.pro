...
IF (N_ELEMENTS(scale) NE 0) THEN BEGIN
 IF ((scale LT 1) OR (scale GT 30)) THEN BEGIN
  ...
  STOP
 ENDIF
 csfile='Z:/Ddisk/IDL/Book/PRO for the book_V4/lib/colour_scales/cs'+SCROP(scale)+'.txt'
 rgb=INTARR(256,3)
...
IF ((N_ELEMENTS(idl) NE 0) OR KEYWORD_SET(VCS)) THEN BEGIN
 IF (N_ELEMENTS(idl) NE 0) THEN BEGIN
  IF ((idl LT 0) OR (idl GT 40)) THEN BEGIN
   ...
   STOP
  ENDIF
  csfile='Z:\Ddisk\IDL\Book\PRO for the book_V4\lib\colour_scales\idl'+SCROP(idl)+'.txt'
 ENDIF
