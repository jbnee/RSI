...
; read,year, prompt='year as 2003: ?  '
 ...
 FOR nm=ni,nf do begin
  ...
  ;ln=strlen(sn)
   ...
   sigm=datab[0:bnum-1]
   sigm=smooth(sigm,10)
   bk=mean(smooth(datab[bnum-200:bnum-1],20))
   pr2m[j,*]=(sigm-1*bk)*ht^2
   x=findgen(nx)
   y=ht1
  ...
  cmax=25; 2*max(pr2m[(nf-ni)/2,0:200])
  cmin=cmax/40.0
  ;nlevs=40
  ;nlevs_max=40
 ...
 s2=strtrim(fix(nf),2)
 S='
 contour,pr2m,x,y,xtitle='Time (min)',ytitle='Height (km)',title=S, yrange=[h1,h2],LEVELS = CLEVS, /FILL,$
    C_COLORS = C_INDEX, color=2,/FOLLOW,charsize=1.3,charthick=1.5,position=plot_position1
; xyouts,1.5*max(x),0.2*max(y),f3,/device
 ...
 FOR nd=ni,nf do begin
  ...
  col = 240 ; don't change it
  cmaxd=2.5*max(pr2d[nf-ni-nx/2,50:200]);300;   0./5
  cmind=cmaxd/40
 ...
 ;CLEVS = CLEVS(WHERE(CLEVS LE CMAX))
  ...
  NLEVS = N_ELEMENTS(CLEVS)+1 ;actual number of countour levels
   ...
   ;!x.range=[ni,ni+nf]
   contour,pr2d,x,y,ytitle='Height (km)',title=dnm, yrange=[h1,h2],LEVELS = CLEVS, /FILL,$
    C_COLORS = C_INDEX, color=2,/FOLLOW,charsize=1.3,charthick=1.5,position=plot_position2
 ...
 ; plot a color bar, use the same clevs as in the contour
  ...
  ;opath='F:\lidar_data\output\'   ;output path Rayleigh\1995\se\se112342
   opath='F:\RSI\lidar\EV_plots\';+yr+'\CONTOUR\';
    cname =opath+yr+'_'+dnm+'wave.png'
