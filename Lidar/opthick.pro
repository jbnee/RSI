...
;Part 2 Read data
  ...
  Data_path='f:\Lidar_data\';  2003\jn022058\jn022058.4M""   ; AUR or SPR

  yrmn='';   year\month
  Read,yrmn, prompt='Enter director Year, month as 2003\JN:  '
  dirnm=data_path+yrmn
   out_path='f:\Lidar_data\out2002\';+yr+'\output\"
  ...
  Read, dnm, PROMPT='Enter dnm as ja162136se010044:/se112342 '   ; Enter date+code
  ...
  For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
    ...
    fn=dirnm+'\'+dnm+'.'+ni+'m'              ;+Dtype ;

    out_file=data_path+dnm+'_prf_.'+'bmp'
     ;openr,1,data_file;
     ...
     if (Jr eq 0) then begin
        plot,bX0[jr,*],ht,color=2,background=-2,yrange=[h1,h2],title='file'+S,xtitle= 'file number',ytitle='km';file 1',color=2
     endif else begin
     oplot,bX0[jr,*]+0.01*pX0*jr,ht,color=2;
...
; evaluation of the second bottom term below
  For nj=0,n5-1  do begin  ;from starting file n1 to last file n2
   ...
   ht1=ht[lo_bin:hi_bin]
   pk=max(beta_a[0,*])
    ;plot,beta_a[0,0:hi_bin],ht,yrange=[h1,h2],background=-2, color=2, title='beta_a'
    plot,beta_a[0,*],ht1,yrange=[h1,h2],xrange=[0,2*pk],background=-2,color=1,xtitle='beta_a'

      stop; section IIA

    For k=0,n5-1 do begin
         oplot,beta_a[k,*],ht1,color=2  ;ht[100:hi_bin-100]

    ENDFOR ;k
      out_file1=data_path+dnm+'beta_.'+'bmp'
    ...
    bratio[0,0:hi_bin]=1+smooth(beta_a[0,0:hi_bin]/beta_r[0:hi_bin],10)
     plot,bratio[0,0:hi_bin],ht1,xrange=[0,50],background=-2,color=1,title=dnm,xtitle='bacattering ratio 0',ytitle='km'
      ...
      sumratio=0
       ...
       oplot,bratio[L,0:hi_bin],ht1,color=1  ;hi_bin-100
       ;stop
     ...
     oplot,avratio,ht1,color=4,linestyle=2,thick=5
     stop    ;Section III
      out_file2=out_path+dnm+'ratio_.'+'bmp'
