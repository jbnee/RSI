Pro Licel_2020_2channel_counting
...
!P.COLOR=2 ;blue labels
 ...
 ; CPPD=A[0:m-1,*] ; analog
  ...
  cmax=2*max(CPPM1[m/3,bn1:bn2]);for high altitude  ;cmax=1*max(CPPM[m/2,100:1000]); for low altitude
...
;;;;;;;;;;;;separate two channels;;;;;;;;;;;;;;;;;;;;
   ...
   contour,CPPM1,x,y,xtitle='time starting :'+T0,ytitle='km',yrange=[0,h2],$
   LEVELS = CLEVS, /FILL, C_COLORS = C_INDEX ,/FOLLOW,charsize=1.4, position=pA,$
   title=da+'perp polarization'

   ;xyouts,500,5,'fn',color=1,charsize=2
   ;C_INDEX=C_INDEX*2.5
   contour,CPPM2,x,y,ytitle='km',yrange=[0,h2],LEVELS = CLEVS, /FILL,$
    C_COLORS = C_INDEX,/FOLLOW,charsize=1.4, position=pB,title=da+'pll polarization time:'+T0+'-'+T9
 ...
 stop
 outpath2='E:\lidar_dataa\2020\CONTOUR\'
 fx=da+'_counting.Png'
out2=outpath2+fx
