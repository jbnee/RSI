 ...
 S='                                       file:'+s1+'_'+s2
  ...
  ht=3.0E8*160.E-9*T/2./1000.   ;convert ht to km
   ...
   ;fn1=fx+ '1m'
    ...
    mina=min(Pr2m[j,*])
    dv=maxa/nx
    plot,pr2m[j,*],ht1,background=-2,color=2,position=plot_position1,xrange=[0,1*maxa],yrange=[0,3],$
    xtitle='m channel',ytitle='km' ,title=S
...
k=0
  ...
  dv2=maxb/nx
    ...
    xrange=[0,maxb],yrange=[0,3],xtitle='d channel',ytitle='km' ,Title=fnm

  k=1

 FOR nd=ni+1,nf do begin
   sn=strtrim(fix(nd),2)
   fn=fnm+strtrim(sn,2)+'D'
   datab=read_binary(fn,DATA_TYPE=2)
   sig2=datab[0:1999]
   sigd=smooth(sig2,10)
   pr2d[k,*]=sigd*ht^2
...
write_bmp,'d:\lidarPro\depolar\contour\'+dnm+".bmp",tvrd()
stop
end
