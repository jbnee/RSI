...
opath='d:\Lidar systems\Raman lidar\'
path='d:\Lidar systems\Raman lidar\2009\ja\'
...
sig1=fltarr(4000)
sig2=fltarr(4000)
ht1=3.0E8*50.E-9*T/2./1000.+0.001   ;convert ht to km and remove 0
;ht1=2*7.5*T/1000  ; ht in km with resoluton 7.5m
 ...
 n1=round(h1*1000./15.)  ;channel number
 n2=round(h2*1000./15)  ;ch
...
i0=1
 ...
 x=findgen(nx)
 contour,pr2m,x,ht1,xtitle='time',ytitle='km', title=S,   yrange=[h1,h2],LEVELS = CLEVS, /FILL,$
 ...
 FOR nd=ni+1,nf do begin
  ...
  ENDFOR
  col = 240 ; don't change it
cmaxd=max(pr2d[10,0:999])
cmind=min(pr2d[10,0:999])
nlevs_max=20
nlevs_max = 20 ; choose what you think is right
 ...
 NLEVS = N_ELEMENTS(CLEVSd)+1 ;actual number of countour levels
   ...
   contour,pr2d,x,ht1,xtitle='time, d channel',ytitle='km',title=dnm,yrange=[h1,h2],LEVELS = CLEVS, /FILL,$
 ...
 ; plot a color bar, use the same clevs as in the contour
  ...
  cntrname =opath+'figures\'+dnm+'.png'
