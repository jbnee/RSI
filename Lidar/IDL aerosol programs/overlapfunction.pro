...
;file_basename1='D:\LidarPro\Data\2002mr\mr192028.'; mr182309.
;mdx=''
...
; read,ni,prompt='which file number to process such as 5',
  ...
  ; fn=file_basename1+string(n,format='(1(I2))')+'m'
 ...
 sig=datab[0:2000]   ;2000 is 48 km
 sumsig=sumsig+sig
 ht1=ht[0:2000]
 if (n EQ ni) then begin
  plot,sig,ht1,background=-2,color=2,xrange=[0,2000],yrange=[5,35],xtitle='Signal/channel',ytitle='km'
  goto,jp1
 endif else begin
 ;goto, JP
  oplot,sig+100*J,ht1, color=2 ;background=-2,color=1,xtitle='Signal/channel',ytitle='km'
 ; xyouts,2000+300*n,30,n,color=1,charsize=1
 endelse
 jp1: wait,0.5  ;wait 0.5 sec
 J=J+1
endfor
 xyouts,100,10,'filename',color=1,charsize=2
 stop
 Avesig=sumsig/(nf-ni+1)
 td=1.5E-8
 Q=Avesig*td
 ;Rvsig=exp(Q*(exp(Q*(exp(Q*(exp(Q*(exp(Q)))))))))
 Rvsig=sqrt(Q)
...
;;;;;;;; density of air is the polynomial fit of height determined from radiosonde
    ...
    y=dblarr(1024)
    y=(avesig[0:1249]*ht1[0:1249]^2)/(transm*beta_r)
    yc=(Rvsig[0:1249]*ht1[0:1249]^2)/(transm*beta_r)
    plot,y,ht1,background=-2,color=2;xrange=[0,10000],yrange=[5,40],xtitle='Signal/channel',ytitle='km'
    oplot,yc,ht1,color=3,psym=3
    stop
    wait,2
     plot,y[100:1200]/y[600],ht1,xrange=[0,5],background=-2,color=2
