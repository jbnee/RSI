...
;path='d:\LidarPro\Rayleigh\1995\se\se112342.';.'
 ...
 FOR n=ni,nf do begin
  ...
  cmax=1000;1.3*max(pr2m[nf-ni-nx/2,50:200]);400;1500;500;./2
 ...
 contour,pr2m,x,y,xtitle='Time (min)',ytitle='Height (km)',yrange=[h1,h2],charsize=1.5,charthick=2,LEVELS = CLEVS, /FILL,$
...
; xyouts,1.5*max(x),0.2*max(y),f3,/device
 ...
 FOR nd=ni+1,nf do begin
  ...
  col = 240 ; don't change it
  cmaxd=1000;0.25*max(pr2d[nf-ni-nx/2,*]);300;   0./5
  cmind=cmaxd/40.
 ...
 ;CLEVS = CLEVS(WHERE(CLEVS LE CMAX))
  ...
  NLEVS = N_ELEMENTS(CLEVS)+1 ;actual number of countour levels
   ...
   ;!x.range=[ni,ni+nf]
   Tnm=dnm+S
   contour,pr2d,x,y,ytitle='Height (km)',title=Tnm, yrange=[h1,h2],charsize=1.5,charthick=2,$
   LEVELS = CLEVS, /FILL,C_COLORS = C_INDEX, color=2,/FOLLOW,position=plot_position2
 ...
 ; plot a color bar, use the same clevs as in the contour
    ...
    for k = 0, nlevs-1 do zb(0:1,k) = cmind + cint*k
     ...
     CONTOUR, zb, xb(0:1), yb(0:nlevs-1),position=BAR_POSITION2,$
      ...
      LEVELS = CLEVS0,C_COLOR = C_INDEX, /FILL, ycharsize=0.7, $
      xcharsize=0.8,/noerase
   xyouts,400,320,color=20,Tnm,/device

  stop

  DEVICE, DECOMPOSED=0
  opath='F:\RSI\lidar\Fernald\new2019\'   ;output path Rayleigh\1995\se\se112342
