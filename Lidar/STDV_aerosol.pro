Pro STDV_Aerosol  ;profile of percentage standard deviation of profiles
close,/all
lodct=5
device,decomposed=0
!P.MULTI = [0, 2, 2]
...
;filenm ='mr190111.';'mr080015.'; 'mr072150.'; 'mr182309.';  'jn022058.';  'mr312023.';202312.';192330.'; 192028.';190111.'; 182309.';  ap022014.' ;
 ...
 FOR nm=ni+1,nf do begin
  ...
  ;ln=strlen(sn)
   ...
   if min(sig2) le 0 then sig2=sig2-min(sig2)
   pr2d[j,*]=sig2*ht1^2
...
; plot_position6 = [0.1,0.55,0.95,0.95];
 ...
 FOR mp=0,N5-1 do begin
  ...
  plot,pkm,color=25,background=-2,psym=4,xtitle='min',ytitle='Intensity STD_m',position=plot_position1
 ...
 FOR mq=0,N5-1 do begin
  ...
  plot,pkd,color=4,background=-2,psym=4,xtitle='min',ytitle='Intensity STD_d',position=plot_position2,title='Int pk sig & stds'
...
p_stdsm=fltarr(N5)
q_stdsd=fltarr(N5)
A_stdsm=stds_m[*,b2:b1-1]
B_stdsd=stds_d[*,b2:b1-1]
tm=findgen(N5)
