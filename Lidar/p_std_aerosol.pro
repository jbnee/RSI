Pro p_STD_Aerosol  ;profile of percentage standard deviation of profiles
close,/all
!P.MULTI = [0, 2, 2]
...
;filenm ='mr190111.';'mr080015.'; 'mr072150.'; 'mr182309.';  'jn022058.';  'mr312023.';202312.';192330.'; 192028.';190111.'; 182309.';  ap022014.' ;
 ...
 FOR nm=ni+1,nf do begin
  ...
  ;ln=strlen(sn)
   ...
   bk1=mean(datab[bin_z-20:bin_z-2])
   bk2=mean(datab2[bin_z-20:bin_z-2])
   sig1=datab[0:bin_z-1]-bk1                  ;parallel m file
   pr2m[j,*]=sig1*ht1^2

   sig2=datab2[0:bin_z]-bk2               ;perpendicular d files
   pr2d[j,*]=sig2*ht1^2
...
For iy2=0,bin_z-1 do begin
  Meansig_M[iy2]=mean(AVsig_M[0:N5-1,iy2])   ;所有的平均 average of all AVsig m profiles
  Meanp_M=mean(PSTD_M[0:N5-1,iy2])
  Meansig_D[iy2]=mean(AVsig_D[0:N5-1,iy2])    ; av   stdv d files
  Meanp_D=mean(PSTD_D[0:N5-1,iy2])
endfor  ;iy2
pksigm=max(Meansig_M[0:bin_z-1])  ;  peak bin_z is 6 km
htpksigm=where(Meansig_m[0:bin_z-1] eq pksigm)
xm=where(ht1 eq htpksigm)
pksigd=max(Meansig_d[0:bin_z-1])  ;bin_z is 6 km
htpksigd=where(Meansig_d[0:bin_z-1] eq pksigd)
...
plot,PSTD_M[0,*],ht1,background=-2,color=2,xrange=[0,scale2],yrange=[0,z],$
 ...
 FOR mp=1,N5-1 do begin
  oplot,PSTD_M[mp,*]+a2*mp,ht1,color=4
 endfor
stop
