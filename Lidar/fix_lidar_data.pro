...
; In the second part, we will compare bk with stdv to rejct or accep this data
; Part I Rayleigh calcution
 ; read,bnum, prompt='Enter total number bins (eg.4096)  '
  ...
  ;Data_path="D:\lidar_files\Depolar\";\Lidar_systems\LidarPro\DePolar\";  2003\jn022058\jn022058.4M""   ; AUR or SPR
   ...
   out_path='F:\lidar_data\2009\out1\'; lidarPro\output\yrmn"
  yr=''
  ...
  For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
   ...
   ;  out_file1=data_path+'\output\'+'pr2m.txt'
    ...
    ; plot,cnt_sig
     ...
     ;STDV=stddev(sg1[jr,bnum-200:bnum-1])
     ;PK1=max(sg1[jr,bnum-200:bnum-1])

     X10[jr,*]=(sg1[jr,*]-bk1)*(z^2); PR2
     MN1=mean(X10[jr,bnum-3000:bnum-2000])
     PK1=max(X10[jr,bnum-3000:bnum-2000])
     RX[jr]=ABS(PK1/MN1)
...
FOR i=0 , nx-5, 5 do begin; average 5 profiles
 ...
 ;if (PK1 GE ST1) then begin
  ;print,J2,'ST1=  ',ST1,'PK1=  ',PK1
  ;endif

  oplot,X10[J2,0:2000],ht[0:2000],color=5*i
  j2=j2+1
endfor;  i
...
;;;;;;;;;;;;;;;;;input d file;;;;;;;;;;;;;;;;;;;;;;;;;;
 For Jr=0,nx-1  do begin  ;1st For ; automatically read as many files
    ...
    fn2=Data_path+yr+'\'+month+'\'+dnm+'.'+ni+'d'
     ...
     readf,1,cnt_sig2
    close,1
     sg2[Jr,*]=smooth(cnt_sig2,20) ;smooth signal
     bk2=mean(sg2[jr,bnum-200:bnum-1]);

     X20[jr,*]=(sg2[jr,*]-bk2)*(z^2); PR2
     MN2=mean(X20[jr,bnum-3000:bnum-2000])
     PK2=max(X20[jr,bnum-3000:bnum-2000])
     RX[jr]=ABS(PK2/MN2)
