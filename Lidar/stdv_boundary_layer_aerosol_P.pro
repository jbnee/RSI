Pro STDV_Boundary_layer_Aerosol_P  ;night data, profile of percentage standard deviation of profiles
close,/all
erase
lodct=5
device,decomposed=0
...
print,da  ;day of data
   ...
   ;Read,nf1,nf1,prompt='starting and ending file number: '
    ...
    PS0=fltarr(m,Nbin) ;Ps photon counting signal
    PS=fltarr(m,Nbin); background
    ;AS0=fltarr(m,Nbin) ;original As  Analog signal
    ;AS=fltarr(m,Nbin); treated AS signal
    BAk=fltarr(m);  AS background
    PSX=fltarr(m,Nbin)
    RPS=fltarr(m,Nbin)
...
;;;;;;;;;;;;;;;;;;;; reading data below
    ...
    BAk[j]=min(datab[1,5500:5999]); background
    PS0[J,*]=datab[1,*];*(ht*1000)^2
     ; PSB[j]=mean(datab[1,5000:5900]); background
     ; PS[j,*]=PS[j,*]-PSB[j]
    PSx[j,*]=PS0[j,*]-BAK[j]  ;remove background
    close,1
    J=J+1
   ENDFOR;
   ...
   for im=0,m-1 do begin
       Q0=where(PSx[im,*] gt 0)
       PSt=PSx[im,Q0]  ; remove negative and zero terms
       RPS[im,*]=interpol(PSt[q0],ht[q0],ht) ;  refill missing pts
...
;m2=round(m/2)
meanPS1=total(PS1,1)/m;
meanPS2=total(PS2,1)/m
meanPS=meanPS1+meanPS2;
maxa=max(meanPS)
plot,meanPS1,ht,color=2, background=-2,xrange=[0,maxa/20],yrange=[h1,h2],xtitle='count',ytitle='km',$
   title='Fig1-'+XC,charsize=1.5
oplot,meanPS2,ht,color=80; background=-2,xrange=[0,maxp],yrange=[h1,h2],position=pos2,xtitle='time (min)',ytitle='km',$
oplot,meanPS,ht,color=200;
;title=XC,charsize=2
...
;;;;average 5 profile as background to calculate SDS
  FOR i=0,nx/2-5,5 do begin
     ...
     AVSig[k1,*]=total(PS12[i:i+4,*],1)/5 ;average of every 5 m profiles



    For iy=0,Nbin-1 do begin

      STDS[k1,iy]=STDDEV(PS12[i:i+4,iy])  ;standard dv of 5 m profiles
      PSTD[k1,iy]=STDS[k1,iy]/AVSig[k1,iy]   ;percentage standard deviation

    ;STDS_D[k1,iy]=STDDEV(PS21[i:i+4,iy])   ;std of d files
