...
;     file=findfile(fn,count=numfiles)
     ch_d=fltarr(numfiles/2.,4000) &  ch_m=fltarr(numfiles/2.,4000)
;
     FOR ii=0,numfiles/2.-1.,2 DO BEGIN
;J=0
;     FOR ii=0,numfiles-2.,2 DO BEGIN
       filename_d=file(ii)
...
;        ch_d(J,*)=SMOOTH(temp-mean(temp(3500:3999)),11)
;        ch_d(J,*)=temp
...
;       ch_m(J,*)=SMOOTH(temp-mean(temp(3500:3999)),11)
;        ch_m(J,*)=temp
       CLOSE, lun & FREE_LUN, lun
