...
;Part 2 Read data
 ...
 ; Data_path="D:\Lidar systems\Depolar\"
  ...
  EX=fltarr(nx+1,bnum+1)
  B1=fltarr(nx+1,bnum+1)
  B=fltarr(nx+1)  ;background data
  beta_a=dblarr(nx+1,bnum+1)  ;backscattering coefficient of aerosol
  ;bratio=dblarr(nx+1,bnum+1)
  izt=2000  ; izt is the top height
  ...
  For Jr=0,nx  do begin  ;1st For ; automatically read as many files
    ...
    ; stop
     ...
     qu=where(EX_A[Jr,*] GT 0)  ; check for positive
     EX[Jr,*]=EX_A[Jr,qu]; take only positive value
     endfor ;jr
...
; evaluation of the second bottom term below
  ...
  ENDFOR   ;nj
    ...
    N2=dblarr(bnum)
    N2=av_beta/cross_sec
    ;N2=smooth(N2,50)
    plot,N2,ht,color=2,background=-2,yrange=[y1,y2]
   ...
   ;for Iq=1,q0[1]-1 do begin
    ...
    oplot,N2x/1e6,ht,color=2,thick=2


    stop
    plot,n2x/density,ht,color=2,background=-2,yrange=[y1,y2],xtitle='ratio=N2X/density',ytitle='km',title='Klett_Raman'
