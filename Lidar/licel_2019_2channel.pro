...
maxp=max(meanPs1)
plot,meanPs1,ht,color=2, background=1,xrange=[0,maxp],yrange=[h1,h2],xtitle='count',ytitle='km',$
   title=XC,charsize=2
...
FOR ic=0,98 do begin
oplot,smooth(PS1[ic,*],20)+ic*maxp/50,ht,color=2
endfor
...
!P.COLOR=2 ;blue labels
 ...
 ; CPPD=A[0:m-1,*] ; analog
  ...
  cmax=3.0*max(CPPM[m/2,bn1:bn2]);for high altitude  ;cmax=1*max(CPPM[m/2,100:1000]); for low altitude
...
;;;;;;;;;;;;separate two channels;;;;;;;;;;;;;;;;;;;;
   ...
   contour,CPPM1,x,y,xtitle='|| channel time starting 18:04PM',ytitle='km',xrange=[0,100],yrange=[0,h2],$
   LEVELS = CLEVS, /FILL, C_COLORS = C_INDEX/1.5,/FOLLOW,charsize=1.4, position=pA,$
   title=da+'Parallel polarization'

   ;xyouts,500,5,'fn',color=1,charsize=2
   C_INDEX=C_INDEX*2.5
   contour,CPPM2,x,y,ytitle='km',xrange=[0,100],yrange=[0,h2],LEVELS = CLEVS, /FILL,$
    C_COLORS = C_INDEX,/FOLLOW,charsize=1.4, position=pB,title=da+'Perpendicular polarization
