...
;path='d:\LidarPro\Rayleigh\1995\se\se112342.';.'
 ...
 FOR n=ni,nf do begin
  ...
  cmax=1500.;1.5*max(pr2m[nf-ni-nx/2,50:200]);400;1500;500;./2
  cmin=0.;20.; cmax/50; cmax/10;min(pr2m[nf-ni-nx/2,0:200])
...
; xyouts,1.5*max(x),0.2*max(y),f3,/device
 ...
 ; plot a color bar, use the same clevs as in the contour
   ...
   ; nlevs=40
    ...
    for k = 0, nlevs1-1 do zb(0:1,k) = cmin + cint1*k
     ...
     xname = [' ',' ',' ']
     CONTOUR, zb, xb(0:1), yb(0:nlevs1-1),position=BAR_POSITION1,$
 ...
 FOR nd=ni+1,nf do begin
  ...
  col = 240 ; don't change it
  cmaxd=200.;2.5*max(pr2d[nf-ni-nx/2,*]);300;   0./5
  cmind=0.;4.; cmaxd/40.
  nlevs_d=1000.
  cint2 = (cmaxd-cmind)/nlevs_d
  clevs_d = CMIND + FINDGEN(nlevs_d+1)*cint2
 ;clevs_d = clevs_d(WHERE(clevs_d LE CMAX))
  clevs_d0 = clevs_d ; for plot the color bar
  NLEVS = N_ELEMENTS(clevs_d)+1 ;actual number of countour levels
   ...
   ;!x.range=[ni,ni+nf]
   contour,pr2d,x,y,xtitle='d channel time',ytitle='km',title=dnm, yrange=[h1,h2],LEVELS = clevs_d, /FILL,$
    C_COLORS = C_INDEX, color=2,/FOLLOW,position=plot_position2
   ;xyouts,500,5,'fn',color=1,charsize=2
 ;stop
 ; plot a color bar, use the same clevs_d as in the contour
    ;nlevs=40
    zb = fltarr(2,nlevs)
    for k = 0, nlevs-1 do zb(0:1,k) = cmind + cint2*k
     xb = [0,1]
     yb = cmind + findgen(nlevs)*cint2
     xname = [' ',' ',' ']
     CONTOUR, zb, xb(0:1), yb(0:nlevs-1),position=BAR_POSITION2,$
      xrange=[0,1],/xstyle,xtickv=findgen(2),$
      xticks=1,xtickname=xname,yrange =[cmind,cmaxd],/YSTYLE, $
      LEVELS = clevs_d0,C_COLOR = C_INDEX, /FILL, ycharsize=0.7, $
      xcharsize=0.8,/noerase
  stop
