...
;Part I Rayleigh calcution
 ; read,bnum, prompt='Enter total number bins (eg.4096)  '
   ...
   dz=bt*ns*c/2 ;7.5;   24;  bT*ns*c/2  ;increment in height is dz=24 m
    z=dz*findgen(bnum+1)  ;ht in meter
    z[0]=1;   remove 0
...
;Part 2 Read data
 ...
 yr=string(year,format='(I4.4)')
  ...
  B1=fltarr(nA,bnum)
  beta_a=dblarr(nA,bnum)  ;backscattering coefficient of aerosol
  bratio=dblarr(nA,bnum)
    ...
    fn=Data_path+yr+'\'+month+'\'+ dnm+'.'+ni+'m'
     ...
     bk[jr]=mean(datab[bnum-200:bnum-1])
      ...
      FOR Im=0,bnum-1 do begin
        ...
        if (datab[im] LE 0) then datab[im]=mean(bk[jr])
      endfor


      PR2m[jr,*]=(datab(0:bnum-1)-bk[jr])*z^2
      Pr2m[jr,0:20]=pr2m[jr,20]
...
FOR IM=0, nx-AVN,AVN do begin
   ...
   For iy=0,999 do begin  ;average AVN profiles
    AVSig[JmX,iy]=MEAN(pr2m[im:im+avn-1,iy])
   ;oplot,Avsig[Jm ,*],ht,color=45
   ENDFOR  ;iy
    X[JmX,*]=smooth(Alog(Avsig[JmX,*]),10)
   ;FOR ix=0,bnum-1 do begin
    ; if X[JmX,ix] LT 0 then X[JMX,ix]=X[JMX,ix-1]
   ;ENDFOR
    oplot,X[JmX,*],ht,color=2
     WAIT,1
  JMX=JMX+1
  ...
  ENDFOR   ;nj
    ...
    ;plot,beta_a[0,100:izt-1],ht+100/24.0,yrange=[10,20], background=-2,color=1,xtitle='beta_a'
     plot,beta_a[0,*]/maxa,ht,xrange=[0,1.2],yrange=[h1,h2], background=-2,color=1,xtitle='beta_a',$
        title=S1;   =dnm+'                                       file:'+n1+'_'+n2';used to print title
      stop; section IIA
    ...
    close,2
      ...
      ; AOD1[RS2]=0
       ...
       outfile1=out_path+fnm+'AOD_M34_64.bmp'
       write_bmp,outfile1,tvrd(/true)
    ...
    bratio[0,0:izt]=1+smooth(beta_a[0,0:izt]/beta_r[0:izt],10)
     plot,bratio[0,0:izt],ht,XRANGE=[1,200],yrange=[h1,h2],background=-2,color=10;xtitle='back ratio';xrange=[0,100]
      ;bbratio=bratio
      stop  ;IIC
      sumratio=0
    ...
    print,'files 1st last, ave num, total #:',n1,n2,AVn,na
     stop    ;Section III
