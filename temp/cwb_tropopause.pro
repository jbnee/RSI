...
For yue=1, 12 do begin
month=yue

path='d:\Tropopause\cwb06_08\';two '' has been used to indicate that it is string
...
;year=2006 ; for reading the year as an variable quantity (and variable is always string)

;read,year,prompt='Enter which year like 2006 : ';promt is a command to enter variable from desktop
...
;********************  To Sava data file  ******************************************************
 ;
path1='d:\Tropopause\test2\'  ;Users\DasMondal\Desktop\cwb\'
   fout=path1+year+'.'+month+'.'+time+'.txt'
 openw,lun1,fout,/get_lun
printf,lun1,'Date  Temp.   Height'
for j=0,idx-1 do begin
 printf,lun1,j+1,cpt1(j),cpt_h1(j),format='(i2,3x,f8.3,3x,f5.2)'    ;print,h(j),t(j),p(j),hm(j),td(j),wd(j),ws(j),format='(f7.3,2x,f7.2,2x,f7.2,2x,f5.0,2x,f6.2,2x,f6.2)'
endfor
free_lun,lun1
endfor  ; yu
close,/all
