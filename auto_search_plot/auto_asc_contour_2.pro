...
ht=dz*findgen(bnum)/1000.+0.0001   ;convert ht to km and remove 0 as base
 ...
 FLS=file_search(Path1)
 help,FLS

 N_fls=n_elements(FLS); , prompt='Total number files here: '

 READ,N0,prompt='1st file to read, as 1,21:'
 READ,Nz, PROMPT='INPUT NUMBER OF FILES to plot AS 9 '

 YNM=INTARR(Nz)  ; number of data to read
  close,/all
...
pnm=strmid(fls[i1],8,8)
dnm=strcompress(pnm,/remove_all)
!p.multi=[0,5,1]
...
wait,5
;STOP
outpath='D:\lidar_data\'+year+'\contours\'
