...
ht=dz*findgen(bnum)/1000.+0.0001   ;convert ht to km and remove 0 as base
 ...
 read,FILEname,prompt='File name'

 F1=path0+filename+'\*'

 FLS=file_search(F1)  ; FINDING

 nx=n_elements(FLS);
 print,nx

print,'first file: ',fls[0]
print,'Last file:',fls[nx-1]
dnm=''
dnm=strmid(fls[0],19,4)
stop
READ,n1,n2,prompt='1st and last file  '
...
window,1
 ...
 ; CPPD=A[0:m-1,*] ; analog
  ...
  ctest=[cmax1,cmax2]
  cmax=max(ctest)/2  ; select the largest one
