PRO Richardson; read unknow number of elements; H, filename,COLUMNS=cols,ROWS=rows
...
plot,PT,Ht/1000,psym=3
stop
N=50
WSH=interpol(Ws,N);(:,1),Ws(:,2),R(:,1));
plot,WSH,HT/1000
WDH=interpl(Wd,N);(:,1),Wd(:,2),R(:,1))/180*pi;
u=WSH*sin(WDH);
v=WSH*cos(WDH);
stop
DpDz=Deriv(PT,z);
;R(:,2)=9.8.*DZ/PT(:,2)/((u-u(1))^2+(v-v(1))^2)*(PT(:,2)-PT(1,2));
R=9.8/PT/WS^2*(PT(*,2)-PT(1,2))*DpDZ;
stop
end
