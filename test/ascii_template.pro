; $Id: ascii_template.pro,v 1.53 2003/06/11 19:19:09 scottm Exp $
;
...
function at_widget, $
  ...
  if (N_ELEMENTS(group) eq 0) then begin
        ...
        groupCreated = 0B
  base = widget_base(title='ASCII Template', /column, xoff=xoff, yoff=yoff, $
    /modal, group=group, /tlb_frame_attr, space=1)
...
function at_check_file, fname
  ...
  if (error eq 0) then begin
    if !version.os_family eq 'vms' then begin
        free_lun, unit
        return, 0 ; Can't rely on finfo.size, so assume file is OK.
    end
