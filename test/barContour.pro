...
; by He Yujin, 2004
;=======================================================================
pro barContour, z, x, y, levels=levels, nlevels=nlevels, Position=Position, CTLevel=CTLevel, CTBottom=CTBottom, $
  ...
  if not keyword_set(levels) then begin
    if not keyword_set(nlevels) then nlevels=6 else nlevels=nlevels<254
    levels = zmin + dindgen(nlevels) * (zmax-zmin)/(nlevels-1)
  ...
  contour,z,x,y,color=0,background=255,levels=levels,/fill,_extra=_extra,$
  ...
  contour,transpose([[barlevels],[barlevels]]),[0,1],barlevels,/fill,/noerase, $
          ...
          Position=[Position[2]+barOffset[0],Position[1],Position[2]+barOffset[1],Position[3]],$
          charsize=0.6
  loadct,0,/silent
