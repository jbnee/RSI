...
path='d:\Trpopause\cwb90_00\';two '' has been used to indicate that it is string
...
year='1994' ; for reading the year as an variable quantity (and variable is always string)
month=''
time='12'
z1=1
z2=25
...
h_intpl=h(phts(pnan))
    if pindex eq 0 then begin
       plot,t_intpl+273.15,h_intpl,linestyle=0,xrange=[180,240],psym=1,yrange=[10,25],xstyle=1,ystyle=1,$
            background=-1, color=1,YTITLE = 'Height (km)', xtitle='Temperature K' ;,/noerase
       pindex=pindex+1
    endif else begin
    oplot,t_intpl+273.15,h_intpl,linestyle=0,psym=6
    pindex=pindex+1
    endelse
endif
...
;********************  To Sava data file  ******************************************************
 ;
;path1='d:\Tropopause\test3\+'.'+month+'.'+time+'.txt'
; openw,lun1,fout,/get_lun
; printf,lun1,'Date  Temp.   Height'
; for j=0,idx-1 do begin
;    printf,lun1,j+1,cpt1(j),cpt_h1(j),format='(i2,3x,f8.3,3x,f5.2)'
