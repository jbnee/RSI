; $Id: binary_template.pro,v 1.41 2003/06/13 23:34:35 ddirks Exp $
;
...
;
...
endif else begin    ; have a filename
    ...
    if bt_state.fieldcount gt 0 then begin
        ...
        bt_varnames = strcompress(*bt_state.pNames, /remove_all)
        bt_nfield = bt_state.current_field_indx + 1 - (bt_state.we_are_inserting eq 0)
        ...
        for bt_i=0, bt_nfield-1 do begin


            ; Construct result array.

            if (*bt_state.pNumDims)[bt_i] eq 0 then begin ; Scalar.

                bt_tempvar = FIX(0, TYPE=(*bt_state.pTypecodes)[bt_i])

            endif else begin ; Array

                if (bt_state.useExecute) then begin
                    bt_str = 'bt_dimension = ULONG64(' + $
                        rb_dim_str((*bt_state.pDimensions)[bt_i, *], $
                            (*bt_state.pNumDims)[bt_i]) + ')'
                    ...
                    bt_nd = N_ELEMENTS((*bt_state.pDimensions)[bt_i, *])
                    ...
                    for bt_j = 0, bt_nd-1 do begin
                        bt_dim_string = (*bt_state.pDimensions)[bt_i, bt_j]
            ...
            if (~(*bt_state.pAbsoluteflags)[bt_i]) then begin
            ...
            if (bt_state.useExecute) then begin
                bt_void = ROUTINE_NAMES(bt_varnames[bt_i], $
                    TEMPORARY(bt_tempvar), STORE=0)
            endif else begin
    ...
    endif  ; bt_state.fieldcount gt 0
    ...
    if (bt_state.useExecute) then begin
        ...
        bt_str = 'bt_offset = LONG64(' + bt_offset + ')'
        if ~execute(bt_str, 1) then begin
            bt_msg = ['Invalid offset:', !error_state.msg]
            goto, ioErr
        endif
    endif else begin
        ; See if our string matches a var name.
    ...
    if bt_offset_is_relative then begin
        point_lun, -bt_lun, bt_pos ; Get the current position.
        bt_offset = (bt_firstChar ne '<') ? $
            bt_pos + bt_offset : ((bt_pos - bt_offset) > 0)
    endif
    ...
    if bt_num_dims eq 0 then begin

        bt_tempvar = fix(0, TYPE=bt_typecode(bt_state.wType))

    endif else begin

        if (bt_state.useExecute) then begin
            bt_str = 'bt_dimension = ULONG64(' + bt_dims_str + ')'
            if ~execute(bt_str, 1) then begin
                message, /info, 'Error executing string: ' + bt_str
                message, !error_state.msg
            endif
        ...
        bt_tempvar = MAKE_ARRAY(DIMENSION=bt_dimension, $
            TYPE=bt_typecode(bt_state.wType))

    endelse


    if size(bt_tempvar, /n_dimensions) ne bt_num_dims $
        && bt_num_dims gt 0 then begin
        bt_msg = [ $
...
value[2, *] = strcompress(*state.pNumDims, /remove_all)

for i=0,state.fieldcount-1 do begin
    value[3, i] = rb_dim_str( $
        (*state.pDimensions)[i, *], $
        (*state.pNumDims)[i] $
        )
endfor
...
wDummy = WIDGET_LABEL(wOffsetBase, /ALIGN_LEFT, $
    VALUE='The Verify field can contain a number' + expString[0])
wDummy = WIDGET_LABEL(wOffsetBase, /ALIGN_LEFT, VALUE=expString[1])
...
wEndian = widget_droplist( $
    wRowBase, $
    title=' File''s byte ordering:', $
    value=['Native', 'Little Endian', 'Big Endian'] $
    )

wFrameBase = widget_base(tlb, /frame, /col)
void = widget_label(wFrameBase, value='Fields:', /align_left)
case !version.os_family of
    'Windows': column_widths = [100, 50, 50, 100, 70, 50, 50]
    'unix':    column_widths = [100, 70, 80, 100, 70, 80, 80]
    else:      column_widths = [100, 70, 80, 100, 70, 80, 80]
endcase
