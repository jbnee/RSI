...
CLOSE,/ALL
ERASE
NBin=8192; total bin number  8192x3.75=30 km
...
print,da  ;day of data
   ...
   Read,nf1,nf2,prompt='starting and ending file number: '
    ...
    close,/all
    J=0;nf1-1
    read,a_p,prompt='input 0 for analog, 1 for counting: '

   FOR I=nf1,nf2 DO BEGIN ; open file to read
    OPENR,1,fls[I]
...
maxa=max(meanAS1[bn1:bn2-1])
plot,meanAS2,ht,color=2, background=1,xrange=[0,5*maxa],yrange=[h1,h2],xtitle='count',ytitle='km',$
   title=XC+'for every 5 data' ,charsize=1.2

oplot,meanAS1,ht,color=180; background=-2,xrange=[0,maxp],yrange=[h1,h2],position=pos2,xtitle='time (min)',ytitle='km',$
stop
window,1
plot,AS2[0,*],ht,color=2,background=1
for ip=0,m/2-1,5 do begin
oplot,AS2[ip,*]+ip,ht,color=20*ip
oplot,AS1[ip,*]+maxa/2+ip,ht,color=2
endfor
outpath='E:\lidar_data\2020\contour\'
...
!P.COLOR=2 ;blue labels
  ...
  CPPM=AS[na:m-1,*]  ;
  CPPM1=AS1[na:m2-1,*]  ;chan 1 photon counting
  CPPM2=AS2[na:m2-1,*]  ;chan 2  photon counting
  c_size=size(cppm1)
  m3=c_size(1)
 ; CPPD=A[0:m-1,*] ; analog
     ...
     cmax=2*max(CPPM1[m3/2,bn2-1000:bn2-5]);for high altitud
  ;jump1: read,cmax,prompt='input cmax:  '
...
read,hx,prompt='top height must be <h2 :';;Read,nf1,nf1,prompt='starting and ending file number: '
   contour,CPPM2,x,y,xtitle='|| channel time starting 9am',ytitle='km',xrange=[0,m/2],yrange=[0,hx],LEVELS = CLEVS, /FILL,$
   ...
   contour,CPPM1,x,y,ytitle='km',xrange=[0,m/2],yrange=[0,hx],LEVELS = CLEVS, /FILL,$
    C_COLORS = C_INDEX*1, color=2,/FOLLOW,charsize=1.4, position=pB,title= 'Perpendicular polarization'+da+':'+T0+'-'+T9
...
; READ,ANS,prompt='replot? yes input 1: '
 ;if ans NE 0 then goto,junmp1
;;;;;
;;;;;
 stop
 opath='E:\RSI\lidar\output\'
 ;outpath2=opath+'CONTOUR\'
 cont1=da+'_PM_cirrus.Png'
