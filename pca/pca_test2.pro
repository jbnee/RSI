pro PCA_TEST2
close,/all
B=fltarr(4,3)  ;data file
;openr,1,'C:\Users\jb\Google ¶³ºÝµwºÐ\pca'
openr,1,'d:\pca\EX0.txt'
line=''
readf,1,line
readf,1,B
...
N= 3  ; number of observations
   ...
   C=fltarr(m,n)
  sigma=0
  for i=0,m-1  do begin ;i=0:3
   for j=0,n-1 do begin  ;j=0:4
    c(i,j)=B(i,j)-means(i)
    sigma=((B(i,j)-means(i))^2)/(N-1)+sigma
   endfor
   S[i]=sigma
   sigma=0
  endfor
...
stop
   ...
   VA=correlate(A) ;variance array
  CVA=correlate(A,/covariance)
  print,'CVA, covariance ='
  print,CVA
