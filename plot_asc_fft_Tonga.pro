...
outpath='E:\volcano aerosols\Tonga 2022\lidar output\'
...
ntime=[0,200,400,800,1000,1200];
 TAS1=total(AS[ntime[0]:ntime[1]-1,*],1)
;help,TAS1
;TAS1            FLOAT     = Array[8000]
 znm=[0,800,1600,2400,3200,4000]
 TAS1z1=TAS1[znm[0]:znm[1]-1];
 ;stop
 ;TAS1z1=TAS1[ntime[J]+1:ntime[J+1]]

;TAS1Z1          FLOAT     = Array[801]

; TAS1=total(AS[ntime[0]:ntime[1],*],1)
help,TAS1
outpath='d:\volcanos\Tonga 2022\lidar outputs\'
 for Iz=0,4 do begin

   TAS1z1=TAS1[znm[Iz]+1:znm[Iz+1]];  IDL> TAS1z1=TAS1[0:800]
   L=n_elements(TAS1z1)

   Y1=FFT(TAS1z1);

   NZ1=n_elements(Y1)/2.0
   ;help,nz1
   Nf=indgen(Nz1)+1;    400

   power0=abs(Y1/L)^2
   power1=Power0(0:L/2.+1)
   freq=0.5*nf/nz1
   power1=power1[1:nz1-1]
   freq=freq[1:nz1-1]
   peri1=1/freq;
   z=fix(ht[znm[iz+1]])
   plot,peri1,power1,color=2,xrange=[0,nz1-1],background=-2,title=string(IZ)+string(z)+'km'
   xx=string(IZ)+string(z)+'km'
