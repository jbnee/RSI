...
;Path0='E:\Aerosol_dust\Taal_volcano\'
 ...
 ; fx='47918 ROIG IshigakijimaJAN15_12Z.txt';
  ...
  fx='47186 National Typhoon Centre JAN.txt '
...
!p.background=255
WINDOW,0
plot,Temp,km,background=-2,color=1,yrange=[h1,h2],xtitle='Temperature',ytitle='km',charsize=1.5;,position=plotposition1
;plot,WD,km,background=-2,color=1,yrange=[0,10],xtitle='WIND DIRECTION',ytitle='km',charsize=1.5;,position=plotposition3
plot,P,km,color=2,background=-2,xtitle='Pressure hPa',yrange=[h1,h2]
plot,WS,km, xtitle='WIND SPEED',ytitle='km',charsize=1.5;,position=plotposition2
plot,WD,km,background=-2,color=1,yrange=[h1,h2],xtitle='WIND DIRECTION',ytitle='km',charsize=1.5;,position=plotposition4
;
fout=strmid(F1,40,9)
;read,outfile,prompt='name of the outfile as Taals: '
figout1=path0+'FFT_output\'+station+date+'-TPWDWS.bmp'
write_bmp,figout1,tvrd()
;printf,2,meanH
...
oplot,10*anmP,z,psym=2,color=120
;;;;;;;;;;oplot,temp[b1:b2],Z,color=80
print,stddev(ANMP)
print,mean(ANMP)
stop
...
Y1=fft(ANMP)
 ...
 peri=MT*T;   /freq
  !p.multi=[0,1,1]
 plot,peri,xpower,color=2,background=-2,title=Station+' FFT PRESSURE power',$
...
fignm=path0+'FFT_output\'+station+date+'_PRESSURE.bmp'
write_bmp,fignm,tvrd()
F3=path0+'FFT_output\'+station+date+'Pressure.txt'
